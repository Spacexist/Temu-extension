...
async function init() {
  const saved = await chrome.storage.local.get(["planPrompt", "executePrompt", "planTimeout", "executeTimeout", "outputSuffix", "startFromIndex", GPT_TAB_LOCK_KEY, DASHBOARD_STATE_KEY]);
  ...
  for (const input of [els.planPrompt, els.executePrompt, els.startFromIndex, els.planTimeout, els.executeTimeout, els.outputSuffix]) {
...
async function processQueue() {
  ...
  try {
    while (state.running && !state.paused && !state.stopRequested) {
      const nextIndex = state.items.findIndex((item) => !isTerminalStage(item.stage));
      if (nextIndex === -1) {
        state.running = false;
        state.activeIndex = -1;
        addLog("ok", "队列完成。");
  ...
  for (;;) {
    ...
    try {
      ...
      const response = await sendToTab(tab.id, {
        ...
        prompt: els.planPrompt.value.trim() || DEFAULT_PLAN_PROMPT,
...
async function runExecuteStage(item) {
  ...
  for (;;) {
    ...
    try {
      ...
      const response = await sendToTab(tab.id, {
        type: "runExecute",
        prompt: els.executePrompt.value.trim() || DEFAULT_EXECUTE_PROMPT,
...
async function getChatgptTab() {
  const locked = await getLockedChatgptTab().catch(() => null);
  ...
  await setLockedGptTabId(tabs[0].id);
  return tabs[0];
}

async function getLockedChatgptTab() {
  if (!state.gptTabId) {
    throw new Error("尚未锁定 GPT 标签页。");
  }
  const tab = await safeGetTab(state.gptTabId);
  if (!isChatgptUrl(tab.url)) {
    await setLockedGptTabId(null);
    throw new Error("已锁定的 GPT 标签页不再是 ChatGPT 页面。");
  }
  return tab;
}
...
function render() {
  ...
  const remaining = total - success - failed - skipped;
  ...
  els.startFromIndex.disabled = state.running || state.processing;
...
function applyStartFromIndex() {
  ...
  for (let index = 0; index < state.items.length; index += 1) {
    ...
    if (index < startFromIndex - 1) {
      if (!isTerminalStage(item.stage) || item.stage === "skipped") {
        item.stage = "skipped";
        item.error = "";
        skippedCount += 1;
      }
  ...
  if (skippedCount) {
    addLog("info", `已从第 ${startFromIndex} 个开始，跳过前 ${skippedCount} 个。`);
  }
}
