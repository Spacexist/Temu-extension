...
export function importTemplateMainImages(rows, { idPrefix = "template" } = {}) {
  ...
  for (let rowIndex = headerRowIndex + 1; rowIndex < rows.length; rowIndex += 1) {
    ...
    const sourceName = normalizeText(cell(row, 0));
    const sourceNameHash = sourceName ? hashTemplateName(sourceName) : "";
    if (sourceNameHash) {
      if (seenNameHashes.has(sourceNameHash)) {
        duplicateNameRowCount += 1;
        continue;
      }
      seenNameHashes.add(sourceNameHash);
    }
  ...
  return {
    ...
    summary: {
      ...
      uniqueNameCount: seenNameHashes.size,
...
export function hashTemplateName(value) {
  const text = normalizeText(value);
  let hash = 5381;
